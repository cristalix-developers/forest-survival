...
import me.reidj.forest.item.ItemHelper.item
import me.reidj.forest.item.ItemHelper.useItem
    ...
    ROD1(item("⭐ §7Удочка", FISHING_ROD, "ROD1"), mapOf(
        PlayerFishEvent::class.java to BiConsumer { _, it ->
            ...
            if (!ItemHelper.tryUseItem(event.player, WORM1)) {
                it.isCancelled = true
                me.reidj.forest.channel.ModHelper.error(app.getUser(it.player)!!, "Нет червей")
            }
        }
    )),
        ...
        PlayerInteractEvent::class.java to BiConsumer { _, it ->
            ...
            if (event.action == Action.RIGHT_CLICK_BLOCK) {
                if (ItemHelper.tryUseItem(event.player, IRON2)) {
                    ...
                    it.isCancelled = true
                    me.reidj.forest.channel.ModHelper.error(app.getUser(it.player)!!, "Нет металла")
                }
            }
        }
    )),
