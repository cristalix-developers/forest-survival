...
public class RealmsController {
    ...
    private CompletableFuture<List<RealmInfo>> fetchThePitRealms() {
        return ISocketClient.get().<AllRealmsPackage>writeAndAwaitResponse(new AllRealmsPackage()).thenApply(pckg -> Stream.of(pckg.getRealms()).filter(info -> info.getRealmId().getTypeName().equals("PITT")).collect(Collectors.toList()));
    }
