...
import me.reidj.forest.handlers.PackageHandler;
import me.reidj.forest.protocol.*;
import me.reidj.forest.realm.RealmsController;
import me.reidj.forest.socket.ServerSocket;
...
import static me.reidj.forest.socket.ServerSocketHandler.send;
...
public class ForestService {
    ...
    public static final Map<Class<? extends CorePackage>, PackageHandler> HANDLER_MAP = new HashMap<>();
    public static final Map<String, MetricsPackage> METRICS = Maps.newConcurrentMap();
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        try {
            thepitServicePort = Integer.parseInt(System.getenv("FOREST_SERVICE_PORT"));
        } catch (NumberFormatException | NullPointerException exception) {
            System.out.println("No FOREST_SERVICE_PORT environment variable specified!");
            Thread.sleep(1000);
            return;
        }

        PASSWORD = System.getenv("FOREST_SERVICE_PASSWORD");
        if (PASSWORD == null) {
            System.out.println("No FOREST_SERVICE_PASSWORD environment variable specified!");
            Thread.sleep(1000);
            return;
        }
        ...
        registerHandler(MetricsPackage.class, (channel, source, pckg) -> {
        ...
        registerHandler(StatPackage.class, (channel, source, pckg) -> {
            System.out.println("Received UserInfoPackage from " + source + " for " + pckg.getUuid());
        ...
        registerHandler(SaveUserPackage.class, (channel, source, pckg) -> {
            System.out.println("Received SaveUserPackage from " + source + " for " + pckg.getUser());
            userData.save(pckg.getStat());
        });
        registerHandler(BulkSaveUserPackage.class, (channel, source, pckg) -> {
            System.out.println("Received BulkSaveUserPackage from " + source);
            userData.save(pckg.getPackages().stream().map(SaveUserPackage::getStat).collect(Collectors.toList()));
        });
        ...
        registerHandler(RequestJoinPackage.class, ((channel, serverName, thePitPackage) -> {
        ...
        try {
            Javalin.create().get("/", ctx -> ctx.result(createMetrics())).start(Integer.parseInt(System.getenv("FOREST_METRICS_PORT")));
        } catch (NumberFormatException | NullPointerException ignored) {
        }

        Thread consoleThread = new Thread(ForestService::handleConsole);
    ...
    private static String createMetrics() {
        Map<String, MetricsPackage.PacketMetric> metrics = Maps.newHashMap();
        ...
        METRICS.forEach((source, data) -> {
            ...
            data.getMetrics().forEach((key, value) -> metrics.compute(key, (__, old) -> {
                if (old != null) {
     ...
     * @param pckg package
     */
    private static void answer(Channel channel, CorePackage pckg) {
        send(channel, pckg);
    }
     ...
     */
    private static <T extends CorePackage> void registerHandler(Class<T> clazz, PackageHandler<T> handler) {
