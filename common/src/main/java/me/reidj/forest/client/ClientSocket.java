package me.reidj.forest.client;
...
public class ClientSocket extends SimpleChannelInboundHandler<WebSocketFrame> {
    ...
    private final Map<Class<? extends CorePackage>, Consumer<?>> handlersMap = new HashMap<>();
    ...
    protected void channelRead0(ChannelHandlerContext ctx, WebSocketFrame msg) throws Exception {
        if (msg instanceof TextWebSocketFrame) {
            ...
            CorePackage pckg = UtilNetty.readFrame(frame);
    ...
    public <T extends CorePackage> CompletableFuture<T> writeAndAwaitResponse(T pckg) {
        val future = awaitResponse(pckg);
        write(pckg);
        return future;
    }

    public <T extends CorePackage> CompletableFuture<T> awaitResponse(T pckg) {
        CompletableFuture<T> future = new CompletableFuture<>();
        responseCache.put(pckg.getId(), future);
        return future;
    }

    public void write(CorePackage thePitPackage) {
    ...
    public <T extends CorePackage> void registerHandler(Class<T> clazz, Consumer<T> consumer) {
