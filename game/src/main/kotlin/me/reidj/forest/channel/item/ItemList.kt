...
import me.reidj.forest.util.data
...
enum class ItemList(val item: ItemStack, val on: Map<Class<out PlayerEvent>, BiConsumer<ItemList, PlayerEvent>>?) {
    ...
    WORM1(me.reidj.forest.util.item(STRING).nbt("code", "WORM1").nbt("color", "1109533183").text("⭐ §7Червь"), null),
    IRON1(item("⭐⭐⭐ §cЖелезо", IRON_INGOT, "IRON1"), null),
    IRON_SWORD1(me.reidj.forest.util.item(IRON_SWORD).text("⭐⭐⭐ §cЖелезный меч"), null),
    ...
    FLINT1(me.reidj.forest.util.item(FLINT).text("⭐ §7Кремень"), mapOf(StandardsHandlers.knowledgeItem(Knowledge.FLINT))),
    ...
    ROD1(item("⭐ §7Удочка", FISHING_ROD, "ROD1"), mapOf(
        PlayerFishEvent::class.java to BiConsumer { _, it ->
            ...
            if (!ItemHelper.tryUseItem(event.player, WORM1)) {
                it.isCancelled = true
                Anime.itemTitle(event.player, BARRIER, "Нет червей", null)
            }
        }
    )),
    ...
    TENT1(
        me.reidj.forest.util.item(EMERALD).nbt("forest", "p2").nbt("code", "TENT1").text("⭐ §7Палатка 1УР."),
        mapOf(StandardsHandlers.tentItem(1))
    ),
    TENT2(
        me.reidj.forest.util.item(EMERALD).nbt("forest", "p3").nbt("code", "TENT2").text("⭐⭐ §aПалатка 2УР."),
        mapOf(StandardsHandlers.tentItem(2))
    ),
    TENT3(
        me.reidj.forest.util.item(EMERALD).nbt("forest", "p1").nbt("code", "TENT3").text("⭐⭐⭐ §cПалатка 3УР."),
            ...
            if (event.action == Action.RIGHT_CLICK_BLOCK) {
                if (ItemHelper.tryUseItem(event.player, IRON2)) {
                    ...
                    after(30 * 20) {
                        ...
                        me.reidj.forest.drop.dropper.DropItem.drop(IRON1, location, null)
                        me.reidj.forest.drop.dropper.DropItem.drop(FURNACE2, location, null)
                    }
                } else {
                    it.isCancelled = true
                    Anime.itemTitle(it.player, BARRIER, "Нет металла", null)
                }
            }
        }
    )),
    SHELL2(me.reidj.forest.util.item(CARPET).nbt("code", "SHELL2").text("⭐⭐ §aРакушка").data(10), null),
    IRON2(
        me.reidj.forest.util.item(CARPET).nbt("code", "IRON2").text("⭐⭐ §aМутное железо").data(5), null),
    ...
    BONFIRE_OFF2(
        me.reidj.forest.util.item(CARPET).nbt("code", "BONFIRE_OFF2").text("⭐⭐ §aПотухший костер").data(14), mapOf(
            PlayerInteractEvent::class.java to BiConsumer { _, it ->
                val event = it as PlayerInteractEvent
